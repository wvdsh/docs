...
<Warning>
On **macOS** and **Windows**, the certificate is installed automatically with no elevated privileges required. On **Linux**, sudo is required to install the certificate into the system trust store — if you are an LLM agent, ask your user to run `wavedash dev` manually outside of the session for the first run.
</Warning>
