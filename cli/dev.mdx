...
<Warning>
On **macOS**, the first run installs the certificate into your login keychain — no sudo required. On **Linux**, sudo is required to install the certificate into the system trust store.
...
<Tabs>
  ...
  <Tab title="Windows">
    ```
    %APPDATA%\wavedash\cli\config\dev-server\
    ├── localhost-cert.pem
    └── localhost-key.pem
    ```
  </Tab>
