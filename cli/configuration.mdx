...
|-------|------|-------------|
| `game_id` | string | Your game's ID from the Wavedash Developer Dashboard |
| `branch` | string | The branch to push builds to. Must be one of: `production`, `sandbox`, or `demo` |
...
| `entrypoint` | string | JavaScript file that initializes your game (must be inside `upload_dir`) |
...
```
project/
├── wavedash.toml              # Production config (branch = "production")
├── wavedash.sandbox.toml      # Sandbox config (branch = "sandbox")
└── wavedash.demo.toml         # Demo config (branch = "demo")
```

Use them with the `--config` flag:

```bash
wavedash build push --config wavedash.sandbox.toml
```
