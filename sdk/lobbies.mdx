...
// Receive invites via LOBBY_INVITE signal
function handleLobbyInvite(data) {
...
<Note>
The invited user will receive a `LOBBY_INVITE` signal. Your game should handle this signal to show an invitation UI or auto-join the lobby.
</Note>
...
|--------|-------------|------|
| `LOBBY_JOINED` | Successfully joined a lobby | `{ success, lobbyId, hostId?, users?, metadata?, message? }` |
| `LOBBY_USERS_UPDATED` | A user joined or left | `{ id, username, changeType: "JOINED" \| "LEFT" }` |
| `LOBBY_MESSAGE` | New message received | `{ username, content, ... }` |
| `LOBBY_DATA_UPDATED` | Metadata changed | `Record<string, unknown>` (full metadata object) |
| `LOBBY_KICKED` | Removed from lobby | `{ lobbyId, reason }` |
| `LOBBY_INVITE` | Received lobby invitation | `{ lobbyId, inviterId, inviterUsername }` |
