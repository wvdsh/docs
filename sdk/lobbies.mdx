...
```csharp Unity
public void GetLobbyUsers(string lobbyId)
{
    var users = SDK.GetLobbyUsers(lobbyId);
...
```csharp Unity
var count = SDK.GetNumLobbyUsers(lobbyId);
...
{
    SDK.SendLobbyChatMessage(lobbyId, message);
}
```
...
public void SetGameSettings(string lobbyId)
{
    ...
    SDK.SetLobbyData(lobbyId, "mapName", "arena_01");
}
```
...
```csharp Unity
var gameMode = SDK.GetLobbyData(lobbyId, "gameMode");
var mapName = SDK.GetLobbyData(lobbyId, "mapName");
```
