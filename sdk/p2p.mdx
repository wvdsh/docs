---
title: "P2P Networking"
description: "Connect players directly with peer-to-peer WebRTC networking"
---

The P2P API enables direct communication between players using WebRTC. This provides low-latency data transfer for real-time multiplayer games without routing through a server.

## How it works

When players join a lobby, the SDK automatically:

1. Establishes WebRTC connections between all lobby members
2. Sets up reliable and unreliable data channels
3. Handles NAT traversal using TURN servers when needed
4. Notifies your game when connections are ready

## Sending messages

### Broadcast to all peers

<CodeGroup>
```gdscript Godot
func broadcast_to_all(data: PackedByteArray, reliable: bool = true):
    # Empty string for target = broadcast to all peers
    WavedashSDK.send_p2p_message("", data, 0, reliable)

# Example: Send game state
func send_game_state(state: Dictionary):
    var json = JSON.stringify(state)
    var data = json.to_utf8_buffer()
    broadcast_to_all(data, true)
```

```csharp Unity
public void BroadcastToAll(byte[] data, int channel = 0, bool reliable = true)
{
    Wavedash.SDK.BroadcastP2PMessage(data, channel, reliable);
}

// Example: Send game state
public void SendGameState(string jsonState)
{
    var data = System.Text.Encoding.UTF8.GetBytes(jsonState);
    BroadcastToAll(data, channel: 0, reliable: true);
}
```

```javascript JavaScript
WavedashJS.broadcastP2PMessage(
  0,                           // channel
  true,                        // reliable
  new Uint8Array([1, 2, 3, 4]) // payload
);
```
</CodeGroup>

### Send to a specific peer

<CodeGroup>
```gdscript Godot
func send_to_peer(user_id: String, data: PackedByteArray, reliable: bool = true):
    WavedashSDK.send_p2p_message(user_id, data, 0, reliable)
```

```csharp Unity
public void SendToPeer(string userId, byte[] data, int channel = 0, bool reliable = true)
{
    Wavedash.SDK.SendP2PMessage(userId, data, channel, reliable);
}
```

```javascript JavaScript
WavedashJS.sendP2PMessage(
  targetUserId,                // userId (or undefined for broadcast)
  0,                           // channel
  false,                       // reliable
  new Uint8Array([1, 2, 3, 4]) // payload
);
```
</CodeGroup>

### Parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `userId` | string | For unicast | Target peer's user ID |
| `channel` | number | Yes | Channel number (0-7) |
| `reliable` | boolean | No | Use reliable channel (default: true) |
| `payload` | Uint8Array | Yes | Binary data to send |

## Receiving messages

<CodeGroup>
```gdscript Godot
func _process(_delta):
    # Drain all pending messages from channel 0
    var messages = WavedashSDK.drain_p2p_channel(0)
    for msg in messages:
        handle_message(msg)

func handle_message(msg: Dictionary):
    var from_user = msg["identity"]
    var payload: PackedByteArray = msg["payload"]
    var data_string = payload.get_string_from_utf8()
    print("From ", from_user, ": ", data_string)
```

```csharp Unity
private List<Wavedash.P2PMessage> messageBuffer = new List<Wavedash.P2PMessage>();

void Update()
{
    // Drain all pending messages from channel 0
    int count = Wavedash.SDK.DrainP2PChannel(0, messageBuffer);
    for (int i = 0; i < count; i++)
    {
        HandleMessage(messageBuffer[i]);
    }
}

void HandleMessage(Wavedash.P2PMessage msg)
{
    var fromUser = msg.SenderId;
    var payload = msg.Payload;
    var dataString = System.Text.Encoding.UTF8.GetString(payload);
    Debug.Log($"From {fromUser}: {dataString}");
}
```

```javascript JavaScript
const message = WavedashJS.readP2PMessageFromChannel(0);

if (message) {
  console.log(`From: ${message.fromUserId}`);
  console.log(`Data: ${message.payload}`);
}
```
</CodeGroup>

## Channels

The SDK supports up to 8 channels (0-7):

| Channel | Suggested use |
|---------|---------------|
| 0 | Game state updates |
| 1 | Player input |
| 2 | Chat messages |
| 3 | Voice data |
| 4-7 | Custom use |

## Reliable vs unreliable

| Type | Description | Best for |
|------|-------------|----------|
| **Reliable** | Guaranteed delivery, ordered | Important state, chat, game events |
| **Unreliable** | Best-effort, faster | Position updates, input, frequent data |

<CodeGroup>
```gdscript Godot
# Reliable - guaranteed delivery (like TCP)
func send_important_event(data: PackedByteArray):
    WavedashSDK.send_p2p_message("", data, 0, true)

# Unreliable - faster but may drop (like UDP)
func send_position_update(data: PackedByteArray):
    WavedashSDK.send_p2p_message("", data, 1, false)
```

```csharp Unity
// Reliable - guaranteed delivery (like TCP)
public void SendImportantEvent(byte[] data)
{
    Wavedash.SDK.BroadcastP2PMessage(data, channel: 0, reliable: true);
}

// Unreliable - faster but may drop (like UDP)
public void SendPositionUpdate(byte[] data)
{
    Wavedash.SDK.BroadcastP2PMessage(data, channel: 1, reliable: false);
}
```

```javascript JavaScript
// Reliable - guaranteed delivery (like TCP)
WavedashJS.broadcastP2PMessage(0, true, gameStateData);

// Unreliable - faster but may drop (like UDP)
WavedashJS.broadcastP2PMessage(1, false, positionData);
```
</CodeGroup>

## Connection state

Check if peers are ready for messaging:

<CodeGroup>
```gdscript Godot
# Check if we can broadcast to at least one peer
func can_broadcast() -> bool:
    # P2P connections are managed automatically when in a lobby
    # Use events to track connection state
    return true  # Check via p2p_connection_established event
```

```csharp Unity
// P2P connections are managed automatically when in a lobby
// Listen to OnP2PConnectionEstablished to know when peers are ready
```

```javascript JavaScript
// Check if a specific peer is ready
const isReady = WavedashJS.isPeerReady(userId);

// Check if broadcast is ready (at least one peer connected)
const canBroadcast = WavedashJS.isBroadcastReady();
```
</CodeGroup>

## Events

<CodeGroup>
```gdscript Godot
func _ready():
    WavedashSDK.p2p_connection_established.connect(_on_p2p_connected)
    WavedashSDK.p2p_connection_failed.connect(_on_p2p_failed)
    WavedashSDK.p2p_peer_disconnected.connect(_on_p2p_disconnected)

func _on_p2p_connected(payload):
    print("P2P connected to: ", payload["userId"])

func _on_p2p_failed(payload):
    print("P2P connection failed")

func _on_p2p_disconnected(payload):
    print("P2P peer disconnected: ", payload["userId"])
```

```csharp Unity
void Awake()
{
    Wavedash.SDK.OnP2PConnectionEstablished += data => Debug.Log($"P2P connected to: {data["userId"]}");
    Wavedash.SDK.OnP2PConnectionFailed += data => Debug.Log("P2P connection failed");
    Wavedash.SDK.OnP2PPeerDisconnected += data => Debug.Log($"P2P peer disconnected: {data["userId"]}");
}
```

```javascript JavaScript
WavedashJS.addEventListener(WavedashJS.Events.P2P_CONNECTION_ESTABLISHED, (e) => {
  console.log("Connected to peer:", e.detail.username);
});

WavedashJS.addEventListener(WavedashJS.Events.P2P_CONNECTION_FAILED, (e) => {
  console.log("Connection failed:", e.detail.error);
});

WavedashJS.addEventListener(WavedashJS.Events.P2P_PEER_DISCONNECTED, (e) => {
  console.log("Peer disconnected:", e.detail.userId);
});
```
</CodeGroup>

| Event | Description | Data |
|-------|-------------|------|
| `WavedashJS.Events.P2P_CONNECTION_ESTABLISHED` | Peer connected | `{ userId, username }` |
| `WavedashJS.Events.P2P_CONNECTION_FAILED` | Connection failed | `{ userId, username, error }` |
| `WavedashJS.Events.P2P_PEER_DISCONNECTED` | Peer disconnected | `{ userId, username }` |

## Advanced methods

### Message limits

Query the P2P configuration limits at runtime:

```javascript JavaScript
// Max bytes you can send in a single P2P message payload
const maxPayload = WavedashJS.getP2PMaxPayloadSize();
console.log(`Max payload: ${maxPayload} bytes`);

// Max queued incoming messages per channel before oldest are dropped
const maxIncoming = WavedashJS.getP2PMaxIncomingMessages();
console.log(`Max incoming messages per channel: ${maxIncoming}`);
```

These values are derived from the `p2p` configuration passed to `init()`. Use them to validate payload sizes before sending or to tune your message processing loop.

### Pre-allocated message buffer

For performance-sensitive games (especially engine-based), you can use a pre-allocated scratch buffer for outgoing messages to avoid allocations:

```javascript JavaScript
const buffer = WavedashJS.getP2POutgoingMessageBuffer();

// Write your payload into the buffer
buffer[0] = 0x01;
buffer[1] = 0x02;

// Send only the bytes you wrote
WavedashJS.broadcastP2PMessage(0, true, buffer, 2);
```

### Batch-reading messages

For game engines, `drainP2PChannelToBuffer` reads all queued messages from a channel into a tightly packed binary buffer. This is more efficient than reading one message at a time.

```javascript JavaScript
// Drain all messages from channel 0
const packed = WavedashJS.drainP2PChannelToBuffer(0);

// Format: [size:4 bytes LE][payload:N bytes][size:4 bytes LE][payload:N bytes]...
const view = new DataView(packed.buffer);
let offset = 0;
while (offset < packed.byteLength) {
  const size = view.getUint32(offset, true);
  offset += 4;
  const message = packed.subarray(offset, offset + size);
  offset += size;
  // Process message...
}
```

<Note>
JS games can use `readP2PMessageFromChannel()` to get decoded `P2PMessage` objects one at a time. `drainP2PChannelToBuffer` is primarily useful for game engines that need to process messages in bulk.
</Note>

## TURN fallback

When direct connections fail, the SDK automatically falls back to TURN servers for relay.

<Note>
TURN credentials are managed automatically. No configuration required.
</Note>
