---
title: "Remote Storage"
description: "Save and sync player files to the cloud"
---

The Remote Storage API lets you save and load player files to cloud storage. Files persist across sessions and devices, enabling features like cloud saves, user preferences, and synced game data.

## Overview

Remote storage provides:
- **Per-user isolation**: Each player has their own storage namespace
- **Cloud sync**: Files persist across devices and sessions
- **Directory support**: Organize files in folders
- **Metadata access**: Get file size, modification time, and more

## Uploading files

<CodeGroup>
```gdscript Godot
func _ready():
    WavedashSDK.remote_file_uploaded.connect(_on_file_uploaded)

func save_to_cloud(file_name: String):
    # File paths must be absolute and start with OS.get_user_data_dir()
    var path = OS.get_user_data_dir() + "/" + file_name
    WavedashSDK.upload_remote_file(path)

func _on_file_uploaded(response):
    if response.get("success", false):
        print("File uploaded successfully")
```

```csharp Unity
public class CloudSaveExample : MonoBehaviour
{
    public async void SaveToCloud(string localPath)
    {
        var result = await Wavedash.SDK.UploadRemoteFile(localPath);

        if (!string.IsNullOrEmpty(result))
        {
            Debug.Log("File uploaded successfully");
        }
    }
}
```

```javascript JavaScript
// For JS games, first write to IndexedDB, then upload
await WavedashJS.writeLocalFile("saves/slot1.json", new Uint8Array(/* file data */));
const response = await WavedashJS.uploadRemoteFile("saves/slot1.json");

if (response.success) {
  console.log("File uploaded successfully");
}
```
</CodeGroup>

### Example: Save game data

<CodeGroup>
```gdscript Godot
func save_game(slot: int, game_state: Dictionary):
    # Use OS.get_user_data_dir() for absolute paths
    var saves_dir = OS.get_user_data_dir() + "/saves"
    var path = saves_dir + "/slot" + str(slot) + ".json"

    DirAccess.make_dir_recursive_absolute(saves_dir)

    var file = FileAccess.open(path, FileAccess.WRITE)
    file.store_string(JSON.stringify(game_state))
    file.close()

    WavedashSDK.upload_remote_file(path)
```

```csharp Unity
public async void SaveGame(int slot, Dictionary<string, object> gameState)
{
    var path = $"{Application.persistentDataPath}/saves/slot{slot}.json";

    Directory.CreateDirectory(Path.GetDirectoryName(path));
    var json = JsonConvert.SerializeObject(gameState);
    File.WriteAllText(path, json);

    await Wavedash.SDK.UploadRemoteFile(path);
}
```

```javascript JavaScript
async function saveGame(slot, gameState) {
  const data = new TextEncoder().encode(JSON.stringify(gameState));
  const path = `saves/slot${slot}.json`;

  // Write to IndexedDB first
  await WavedashJS.writeLocalFile(path, data);

  // Then upload to remote
  const response = await WavedashJS.uploadRemoteFile(path);
  return response.success;
}
```
</CodeGroup>

## Downloading files

<CodeGroup>
```gdscript Godot
func _ready():
    WavedashSDK.remote_file_downloaded.connect(_on_file_downloaded)

func load_from_cloud(file_name: String):
    # File paths must be absolute and start with OS.get_user_data_dir()
    var path = OS.get_user_data_dir() + "/" + file_name
    WavedashSDK.download_remote_file(path)

func _on_file_downloaded(response):
    if response.get("success", false):
        print("File downloaded successfully")
```

```csharp Unity
public async void LoadFromCloud(string localPath)
{
    var result = await Wavedash.SDK.DownloadRemoteFile(localPath);

    if (!string.IsNullOrEmpty(result))
    {
        Debug.Log("File downloaded successfully");
    }
}
```

```javascript JavaScript
const response = await WavedashJS.downloadRemoteFile("saves/slot1.json");

if (response.success) {
  // File is now in IndexedDB, read it
  const data = await WavedashJS.readLocalFile("saves/slot1.json");
  if (data) {
    const text = new TextDecoder().decode(data);
    const gameState = JSON.parse(text);
  }
}
```
</CodeGroup>

### Example: Load game data

<CodeGroup>
```gdscript Godot
func load_game(slot: int):
    # Use OS.get_user_data_dir() for absolute paths
    var path = OS.get_user_data_dir() + "/saves/slot" + str(slot) + ".json"
    WavedashSDK.download_remote_file(path)

func _on_file_downloaded(response):
    if response.get("success", false):
        var path = response["data"]["path"]
        if FileAccess.file_exists(path):
            var file = FileAccess.open(path, FileAccess.READ)
            var save_data = JSON.parse_string(file.get_as_text())
            file.close()
            apply_save_data(save_data)
```

```csharp Unity
public async Task<Dictionary<string, object>> LoadGame(int slot)
{
    var path = $"{Application.persistentDataPath}/saves/slot{slot}.json";

    var result = await Wavedash.SDK.DownloadRemoteFile(path);

    if (!string.IsNullOrEmpty(result) && File.Exists(path))
    {
        var json = File.ReadAllText(path);
        return JsonConvert.DeserializeObject<Dictionary<string, object>>(json);
    }

    return null;
}
```

```javascript JavaScript
async function loadGame(slot) {
  const path = `saves/slot${slot}.json`;
  const response = await WavedashJS.downloadRemoteFile(path);

  if (response.success) {
    const data = await WavedashJS.readLocalFile(path);
    if (data) {
      const text = new TextDecoder().decode(data);
      return JSON.parse(text);
    }
  }

  return null;
}
```
</CodeGroup>

## Deleting files

Delete a remote file from cloud storage:

<CodeGroup>
```gdscript Godot
func _ready():
    WavedashSDK.remote_file_deleted.connect(_on_file_deleted)

func delete_save(file_name: String):
    var path = OS.get_user_data_dir() + "/" + file_name
    WavedashSDK.delete_remote_file(path)

func _on_file_deleted(response):
    if response.get("success", false):
        print("File deleted successfully")
```

```csharp Unity
public async void DeleteSave(string localPath)
{
    var result = await Wavedash.SDK.DeleteRemoteFile(localPath);

    if (!string.IsNullOrEmpty(result))
    {
        Debug.Log("File deleted successfully");
    }
}
```

```javascript JavaScript
const response = await WavedashJS.deleteRemoteFile("saves/slot1.json");

if (response.success) {
  console.log("File deleted successfully");
}
```
</CodeGroup>

| Parameter | Type | Description |
|-----------|------|-------------|
| `filePath` | string | Path of the remote file to delete |

## Downloading directories

Download all files in a remote directory at once:

<CodeGroup>
```gdscript Godot
func _ready():
    WavedashSDK.remote_directory_downloaded.connect(_on_directory_downloaded)

func download_all_saves():
    var path = OS.get_user_data_dir() + "/saves/"
    WavedashSDK.download_remote_directory(path)

func _on_directory_downloaded(response):
    if response.get("success", false):
        print("All save files downloaded")
```

```csharp Unity
public async void DownloadAllSaves()
{
    var path = $"{Application.persistentDataPath}/saves/";
    var result = await Wavedash.SDK.DownloadRemoteDirectory(path);
}
```

```javascript JavaScript
const response = await WavedashJS.downloadRemoteDirectory("saves/");

if (response.success) {
  console.log("All save files downloaded");
}
```
</CodeGroup>

## Listing directories

<CodeGroup>
```gdscript Godot
func _ready():
    WavedashSDK.remote_directory_downloaded.connect(_on_directory_downloaded)

func list_saves():
    # Directory paths must be absolute and start with OS.get_user_data_dir()
    var path = OS.get_user_data_dir() + "/saves/"
    WavedashSDK.download_remote_directory(path)

func _on_directory_downloaded(response):
    if response.get("success", false):
        for file in response["data"]:
            print(file["name"], " - ", file["size"], " bytes")
```

```csharp Unity
public async void ListSaves()
{
    var path = $"{Application.persistentDataPath}/saves/";
    var files = await Wavedash.SDK.ListRemoteDirectory(path);

    foreach (var file in files)
    {
        Debug.Log($"{file["name"]} - {file["size"]} bytes");
    }
}
```

```javascript JavaScript
const response = await WavedashJS.listRemoteDirectory("saves/");

if (response.success) {
  response.data.forEach(file => {
    console.log(`${file.name} - ${file.size} bytes`);
  });
}
```
</CodeGroup>

### File metadata

```typescript
interface RemoteFileMetadata {
  exists: boolean;      // Whether the file exists
  key: string;          // Full storage key
  name: string;         // Filename relative to directory
  lastModified: number; // Unix timestamp
  size: number;         // Size in bytes
  etag: string;         // Version identifier
}
```

## Path conventions

Paths are relative to the user's storage root:

```
saves/               # Directory for save games
saves/slot1.json     # Save file
settings.json        # Root-level file
replays/             # Directory for replays
```

<Note>
Paths should use forward slashes (`/`) as separators, regardless of platform.
</Note>

<Warning>
**Godot**: File paths must be absolute and start with `OS.get_user_data_dir()`. Do not use `user://` virtual paths—they are not supported by the Remote Storage API.

```gdscript
# Correct
var path = OS.get_user_data_dir() + "/saves/slot1.json"

# Incorrect - will fail validation
var path = "user://saves/slot1.json"
```
</Warning>

## Example: Cloud save system

<CodeGroup>
```gdscript Godot
const SAVE_VERSION = 2

# Use OS.get_user_data_dir() for absolute paths required by Remote Storage API
var save_path: String

func _ready():
    save_path = OS.get_user_data_dir() + "/saves/"

func save(slot: int, game_state: Dictionary) -> void:
    var save_data = {
        "version": SAVE_VERSION,
        "timestamp": Time.get_unix_time_from_system(),
        "state": game_state
    }

    var path = save_path + "slot" + str(slot) + ".json"
    DirAccess.make_dir_recursive_absolute(save_path)

    var file = FileAccess.open(path, FileAccess.WRITE)
    file.store_string(JSON.stringify(save_data))
    file.close()

    WavedashSDK.upload_remote_file(path)
```

```csharp Unity
private const int SaveVersion = 2;

public async Task<bool> Save(int slot, Dictionary<string, object> gameState)
{
    var saveData = new Dictionary<string, object>
    {
        { "version", SaveVersion },
        { "timestamp", DateTimeOffset.UtcNow.ToUnixTimeSeconds() },
        { "state", gameState }
    };

    var path = $"{Application.persistentDataPath}/saves/slot{slot}.json";
    Directory.CreateDirectory(Path.GetDirectoryName(path));

    var json = JsonConvert.SerializeObject(saveData);
    File.WriteAllText(path, json);

    var result = await Wavedash.SDK.UploadRemoteFile(path);
    return !string.IsNullOrEmpty(result);
}
```

```javascript JavaScript
class CloudSaveSystem {
  constructor() {
    this.saveVersion = 2;
  }

  async save(slot, gameState) {
    const saveData = {
      version: this.saveVersion,
      timestamp: Date.now(),
      state: gameState
    };

    const path = `saves/slot${slot}.json`;
    const data = new TextEncoder().encode(JSON.stringify(saveData));

    // Write to IndexedDB first
    await WavedashJS.writeLocalFile(path, data);

    // Then sync to cloud
    const response = await WavedashJS.uploadRemoteFile(path);
    return response.success;
  }

  async load(slot) {
    const path = `saves/slot${slot}.json`;
    const response = await WavedashJS.downloadRemoteFile(path);

    if (!response.success) {
      return null;
    }

    const data = await WavedashJS.readLocalFile(path);
    if (!data) {
      return null;
    }

    const text = new TextDecoder().decode(data);
    return JSON.parse(text).state;
  }
}
```
</CodeGroup>
